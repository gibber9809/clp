        ...
        switch (cur_value.type()) {
            ...
            case ondemand::json_type::string: {
                ...
                if (value.find(' ') != std::string::npos) {
                    node_id = m_schema_tree->add_node(node_id_stack.top(), NodeType::ClpString, "");
                } else {
...
void JsonParser::parse_array_obj(ondemand::array array, int32_t parent_node_id) {
    ...
    for (; it != array.end(); ++it) {
        ...
        switch (cur_value.type()) {
            case ondemand::json_type::object: {
                node_id = m_schema_tree->add_node(parent_node_id, NodeType::Object, "");
                parse_obj_in_array(std::move(cur_value.get_object()), node_id);
                break;
            }
            case ondemand::json_type::array: {
                node_id = m_schema_tree->add_node(parent_node_id, NodeType::StructuredArray, "");
            ...
            case ondemand::json_type::string: {
                ...
                if (value.find(' ') != std::string::npos) {
                    node_id = m_schema_tree->add_node(parent_node_id, NodeType::ClpString, "");
                } else {
            ...
            case ondemand::json_type::null: {
                node_id = m_schema_tree->add_node(parent_node_id, NodeType::NullValue, "");
                m_current_schema.insert_unordered(node_id);
                break;
            }
        }
